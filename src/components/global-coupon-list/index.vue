	...
	<div class="global-coupon-list-wraper">
		...
		<div class="mb10">
			...
			<div class="list" id="contry-list" v-bind:class="{'dis-block': isCountry, 'dis-none': !isCountry}">
				<a href="javascript:;" v-for="(v, k) in countryList" :key="k" v-on:click="showCoupons(v.id, classifyId);" v-bind:class="{'this-type': countryId==v.id}"><span>{{v.site_name}}</span></a>
			</div>
	...
	export default {
		...
		methods: {
			...
			showCoupons (countryId, classifyId) {
				let currentCountryName = this.countryList ? this.countryList[countryId-1].site_name : '不限';
				let currentClassifyName = this.classifyList ? this.classifyList[classifyId-1].classify_name : '项目';
