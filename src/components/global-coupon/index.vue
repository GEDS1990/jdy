<template>
	...
	<div class="global-coupon-wraper">
		...
		<div class="contry-list-wraper" id="contry-list" v-bind:class="{'dis-none': !isCountryList, 'dis-block': isCountryList}" v-if="countryList !== null">
			<a href="javascript:;" v-for="(v, k) in countryList" :key="k" v-on:click="chooseCountry(v.id);" class="this-country"><span>{{v.site_name}}</span></a>
		</div>
		...
		<div class="hot-wraper">
			<div class="title">
				...
				<span class="title-center">热门项目</span>
	...
	export default {
		...
		methods: {
			...
			showCountryList () {
				if (!this.countryList) {
					this.$http.post('/globalCoupon/countryList', {emulateJSON: true}).then((result) => {
						...
						this.currentCountry = this.countryList[0].site_name;
					});
				}
			...
			chooseCountry (countryId) {
				...
				this.currentCountry = this.countryList[countryId-1].site_name;
